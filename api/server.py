...
try:
    ...
    _log.warning(f"executive_reports unavailable: {_import_err}")
    def generate_executive_pdf_report(*a, **kw): return b"Report generation unavailable"
    def generate_excel_export(*a, **kw): return b"Excel export unavailable"

try:
    from services.ecommerce_parser import audit_digital_listing
except Exception as _import_err:
    import logging as _log
    _log.warning(f"ecommerce_parser unavailable: {_import_err}")
    def audit_digital_listing(*a, **kw):
        raise HTTPException(status_code=503, detail="E-commerce audit is not available")

try:
    from services.pdf_generator import generate_compounding_notice_pdf, generate_improvement_notice_pdf
except Exception as _import_err:
    import logging as _log
    _log.warning(f"pdf_generator unavailable: {_import_err}")
    def generate_compounding_notice_pdf(*a, **kw): return b"PDF generation unavailable"
    def generate_improvement_notice_pdf(*a, **kw): return b"PDF generation unavailable"
...
def api_health_alias():
    return {"status": "ok", "timestamp": datetime.utcnow().isoformat(), "demo_mode": DEMO_MODE}
...
async def gemini_health_check():
    ...
    if not DEMO_MODE:
        try:
            # Valid 32x32 blank JPEG for health check to avoid INVALID_ARGUMENT
            from PIL import Image
            _img = Image.new('RGB', (32, 32), color='white')
            _buf = io.BytesIO()
...
async def pre_audit_endpoint(
    ...
    return PreAuditResponse(
        overall_status=overall_status,
        rules=rules,
        estimated_fine_inr=fine_info.estimated_fine_inr if hasattr(fine_info, 'estimated_fine_inr') else 0,
        fssai_verification=fssai_info
    )
