...
def verify_database():
    db_candidates = [
        Path(__file__).parent / "inspections.db",
        Path("backend/inspections.db"),
        Path("inspections.db")
    ]
...
def verify_endpoints(client: TestClient):
    ...
    response = client.post(
        "/api/v1/pre-audit",
        json={"artwork_text": "Mfg by ABC Ltd 110001. Net Qty 500g MRP Rs. 50 (incl. of all taxes) care@abc.com"},
        headers=headers
    )
    if response.status_code != 200:
...
def verify_fixtures():
    ...
    base_dir = Path(os.path.join(os.path.dirname(__file__), "..", "frontend", "src", "fixtures"))
    ...
    for fixture in fixtures:
        ...
        if "overall_status" not in data or "rules" not in data:
            raise ReadinessError(f"Fixture {fixture} is missing required audit fields ('overall_status', 'rules').")
