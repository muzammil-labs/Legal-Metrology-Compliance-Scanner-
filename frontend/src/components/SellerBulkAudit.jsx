...
import { Store, Upload, CheckCircle2, AlertTriangle, QrCode, ShieldCheck, Download, Sparkles } from 'lucide-react';
    ...
    <section className="seller-view">
      <div className="dash-header">
        <div>
          ...
          <p className="mono">Pre-listing Catalogue Audit & Trust Badge Issuance</p>
        </div>
      </div>
      ...
      <div className="seller-grid">
        ...
        <div className="seller-upload-panel">
          ...
          <div className="batch-dropzone">
            ...
            <p>Select multiple SKU packaging photos or label artworks (PNG, JPG, ZIP)</p>
            <input
              type="file"
              multiple
              accept="image/*"
          ...
          {files.length > 0 && (
            <div className="file-list-preview">
              ...
              <div className="file-chips">
                {files.map((f, i) => (
                  <span className="chip" key={i}>{f.name}</span>
                ))}
              </div>
            </div>
          )}
          ...
          >
            <Store size={18} /> {loading ? 'Auditing Catalogue SKUs...' : `Batch Audit ${files.length} SKUs`}
          </button>
        </div>
          ...
          <div className="badge-preview-card">
            <div className="verified-badge">
              ...
              <div className="badge-text">
                ...
                <small className="mono">Digital Cert ID: LM-CERT-2026-DOCA</small>
              </div>
            ...
            <p className="badge-desc">
              Embed this verified SVG trust badge and tamper-evident QR code directly into product listings on Amazon, Flipkart, or Blinkit to boost buyer conversion.
            </p>

            <div className="embed-code-box">
              <code>&lt;img src="https://metrology.doca.gov.in/badge/LM-2026.svg" alt="Legal Metrology Compliant" /&gt;</code>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="docket-panel mt-6">
          <div className="panel-head">
            ...
            <span className="badge pass">
              <CheckCircle2 size={15} /> {batchResult.passed_skus} / {batchResult.total_skus} SKUs Passed
            </span>
          </div>
              ...
              <tbody>
                {batchResult.items.map((item, idx) => (
                  <tr key={idx}>
                    ...
                    <td>
                      <span className={`status-pill ${item.overall_status === 'PASS' ? 'pass' : 'fail'}`}>
                        {item.overall_status}
                      </span>
                    </td>
                    <td><span className="trust-pill">{item.trust_score}</span></td>
                    <td>{item.violation_count === 0 ? '0 Flags (Clean)' : `${item.violation_count} Violations`}</td>
                    <td>
