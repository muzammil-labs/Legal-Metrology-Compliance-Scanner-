...
describe("api.js", () => {
  ...
  describe("loadPrecachedFixture", () => {
    ...
    it("throws an error for unknown fixtures", () => {
      expect(() => loadPrecachedFixture("unknown")).toThrow(
  ...
  describe("executeScanWithCircuitBreaker", () => {
    ...
    it("throws error if no image file is provided for live scan", async () => {
      await expect(executeScanWithCircuitBreaker(null)).rejects.toThrow(
        "An image file is required for live scanning.",
      );
    });
    ...
    it("falls back to control_pass and saves offline on fetch failure", async () => {
      ...
      expect(offlineStorage.saveOfflineScan).toHaveBeenCalledWith(
        mockFile,
        "Test OCR",
        result,
      );
  ...
  describe("executeBatchScan", () => {
    it("calls fetch with form data containing all files", async () => {
      ...
      const files = [new File(["c1"], "f1.png"), new File(["c2"], "f2.png")];
    ...
    it("throws error if response is not ok", async () => {
      ...
      await expect(executeBatchScan([new File([], "a")])).rejects.toThrow(
        "HTTP 400",
      );
    });
  });
  ...
  describe("getNoticeDownloadUrl", () => {
    it("returns correct url", () => {
      expect(getNoticeDownloadUrl("123")).toBe(
        "/api/inspections/123/export-notice?notice_type=COMPOUNDING",
      );
