  ...
  return <main className="shell">
    ...
    <section className="workspace">
      <CameraScanner
        ...
        modes={modes}
      />
      <div className="results-panel"><div className="panel-head"><span>02 / AUDIT RESULT</span><span className={failed ? 'badge fail' : 'badge pass'}>{failed ? <AlertTriangle size={15} /> : <CheckCircle2 size={15} />} {audit.overall_status}</span></div><div className="result-hero"><span className="result-number">{audit.rules.filter(rule => rule.status === 'PASS').length.toString().padStart(2, '0')}</span><div><h2>Rules passed</h2><p>Out of {audit.rules.length} deterministic checks</p></div></div><div className="rule-list">{audit.rules.map(rule => <div className="rule" key={rule.rule}><span className={rule.status === 'PASS' ? 'rule-icon pass' : 'rule-icon fail'}>{rule.status === 'PASS' ? <CheckCircle2 size={15} /> : <AlertTriangle size={15} />}</span><div><strong>{rule.rule}</strong><p>{rule.reason}</p></div><span className="rule-status">{rule.status}</span></div>)}</div></div>
    </section>
  </main>;
}
