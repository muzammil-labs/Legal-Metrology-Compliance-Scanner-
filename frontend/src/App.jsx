...
import { AlertTriangle, CheckCircle2, FileScan, Upload, LayoutDashboard } from 'lucide-react';
...
export default function App() {
  ...
  const [view, setView] = useState('scanner'); // 'scanner' | 'dashboard'
  ...
  return <main className="shell">
    <header className="topbar">
      <div className="brand">
        <span className="brand-mark"><FileScan size={20} /></span>
        <div><strong>METROLOGY / 01</strong><span>FIELD AUDIT CONSOLE</span></div>
      </div>
      <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
        <nav className="mode-switch" style={{ padding: '2px' }}>
          <button className={view === 'scanner' ? 'selected' : ''} onClick={() => setView('scanner')} style={{ padding: '0 12px', minHeight: '32px' }}><FileScan size={14} style={{ marginRight: '6px', verticalAlign: 'middle' }}/>Scanner</button>
          <button className={view === 'dashboard' ? 'selected' : ''} onClick={() => setView('dashboard')} style={{ padding: '0 12px', minHeight: '32px' }}><LayoutDashboard size={14} style={{ marginRight: '6px', verticalAlign: 'middle' }}/>Dashboard</button>
        </nav>
        <div className="status-dot"><i /> SYSTEM ONLINE</div>
      </div>
    </header>
    ...
    {view === 'scanner' ? (
      ...
      <>
        <section className="intro"><div><p className="eyebrow">INSPECTOR ANALYTICS</p><h1>Regional insights.<br /><em>Data driven.</em></h1><p className="lede">Aggregated metrics and non-compliance distribution.</p></div></section>
