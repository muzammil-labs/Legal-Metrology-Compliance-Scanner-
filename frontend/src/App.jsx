...
import { AlertTriangle, CheckCircle2, FileScan, Upload, X } from 'lucide-react';
...
export default function App() {
  ...
  async function runScan() {
    ...
    try {
      setAudit(await executeScanWithCircuitBreaker(file, mode));
      setMessage(mode ? 'Reference fixture loaded' : 'Inspection complete');
  ...
  return <main className="shell">
    ...
    <section className="workspace"><div className="capture-panel"><div className="panel-head"><span>01 / CAPTURE SURFACE</span><span className="mono">{message}</span></div><div className="viewport"><div className="grid-lines" />{loading && <div className="scanline" />}{mode && <div className="fixture-label">{modes.find(item => item.key === mode)?.label}</div>}<div className="viewport-center"><Upload size={28} /><p>{file ? file.name : 'Upload a package label'}</p><small>Camera input or image file</small></div><input type="file" accept="image/*" capture="environment" onChange={event => { setFile(event.target.files?.[0] ?? null); setMode(null); }} /></div><button className="scan-button" disabled={loading || (!file && !mode)} onClick={runScan}><FileScan size={18} /> {loading ? 'SCANNING' : 'RUN COMPLIANCE SCAN'}</button></div><div className={`mobile-drawer-overlay ${drawerOpen ? 'open' : ''}`} onClick={() => setDrawerOpen(false)} />
    <div className={`results-panel ${drawerOpen ? 'open' : ''}`}>
      <button className="close-drawer" onClick={() => setDrawerOpen(false)}><X size={18} style={{verticalAlign:'middle'}}/> Close Results</button>
      <div className="panel-head"><span>02 / AUDIT RESULT</span><span className={failed ? 'badge fail' : 'badge pass'}>{failed ? <AlertTriangle size={15} /> : <CheckCircle2 size={15} />} {audit.overall_status}</span></div>
      <div className="result-hero"><span className="result-number">{audit.rules.filter(rule => rule.status === 'PASS').length.toString().padStart(2, '0')}</span><div><h2>Rules passed</h2><p>Out of {audit.rules.length} deterministic checks</p></div></div>
      <div className="rule-list">{audit.rules.map(rule => <div className="rule" key={rule.rule}><span className={rule.status === 'PASS' ? 'rule-icon pass' : 'rule-icon fail'}>{rule.status === 'PASS' ? <CheckCircle2 size={15} /> : <AlertTriangle size={15} />}</span><div><strong>{rule.rule}</strong><p>{rule.reason}</p></div><span className="rule-status">{rule.status}</span></div>)}</div>
      {audit.penalty && (
        <div className="penalty-box">
          <h3>Penalty Estimate</h3>
          <p>Violations: {audit.penalty.sections_violated.join(', ')}</p>
          <p>Fine Range: {audit.penalty.estimated_fine_range}</p>
        </div>
      )}
    </div></section>
  </main>;
