...
def test_evaluate_pdp_font_sizes_under_50():
    from services.pdp_geometry import evaluate_pdp_font_sizes
    from schemas import RuleStatus
    res = evaluate_pdp_font_sizes(bounding_boxes=[{"ymin": 0.0, "ymax": 0.005}], image_dimensions={"width_cm": 5.0, "height_cm": 10.0})
    assert res.font_size_compliance == RuleStatus.FAIL

def test_evaluate_pdp_font_sizes_50_to_100():
    from services.pdp_geometry import evaluate_pdp_font_sizes
    from schemas import RuleStatus
    res = evaluate_pdp_font_sizes(bounding_boxes=[{"ymin": 0.0, "ymax": 0.01}], image_dimensions={"width_cm": 10.0, "height_cm": 10.0})
    assert res.font_size_compliance == RuleStatus.FAIL

def test_evaluate_pdp_font_sizes_100_to_500():
    from services.pdp_geometry import evaluate_pdp_font_sizes
    from schemas import RuleStatus
    res = evaluate_pdp_font_sizes(bounding_boxes=[{"ymin": 0.0, "ymax": 0.25}], image_dimensions={"width_cm": 20.0, "height_cm": 20.0})
    assert res.font_size_compliance == RuleStatus.PASS

def test_evaluate_pdp_font_sizes_over_500():
    from services.pdp_geometry import evaluate_pdp_font_sizes
    from schemas import RuleStatus
    res = evaluate_pdp_font_sizes(bounding_boxes=[{"ymin": 0.0, "ymax": 0.01}], image_dimensions={"width_cm": 30.0, "height_cm": 20.0})
    assert res.font_size_compliance == RuleStatus.FAIL
