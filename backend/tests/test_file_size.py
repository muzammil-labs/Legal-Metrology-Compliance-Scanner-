...
def get_auth_headers():
    token = create_access_token(data={"sub": "testuser", "role": UserRole.CENTRAL_ADMIN.value})
...
async def test_file_upload_limit_batch_scan():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        large_content = b"x" * (10 * 1024 * 1024 + 1) # 10MB + 1 byte
        response = await ac.post("/api/scan/batch", headers=get_auth_headers(), files={"files": ("test.jpg", io.BytesIO(large_content), "image/jpeg")})
        assert response.status_code == 413
        assert response.json() == {"detail": "File size exceeds 10 MB limit"}
...
async def test_file_upload_success_scan():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        response = await ac.post("/api/scan", headers=get_auth_headers(), files={"file": ("test.jpg", io.BytesIO(small_content), "image/jpeg")})
...
async def test_file_upload_success_batch_scan():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        small_content = b"x" * 1024 # 1KB
        response = await ac.post("/api/scan/batch", headers=get_auth_headers(), files={"files": ("test.jpg", io.BytesIO(small_content), "image/jpeg")})
        assert response.status_code != 413
